...
function object_proto:ResetState(doNotRefresh)
	...
	self:SetAtTop(id == self:GetNumHistoryElements() and
		self:GetLastActiveMessageOffset() < self:GetMessageLineHeight())
...
do
	...
	local slidingMessageFramePool = CreateUnsecuredObjectPool(
		function(pool)
			local frame = Mixin(
				CreateFrame("ScrollFrame", "mUIFrame" .. curID, UIParent, "mUIHyperlinkPropagator"), object_proto)
