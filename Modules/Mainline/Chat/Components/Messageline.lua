...
do
    ...
    function message_line_proto:AdjustHeight()
        -- Like Chattynator, use GetLineHeight() as the safe base instead of GetStringHeight()
        -- GetLineHeight() returns the font's line height and is never a secret value
        -- GetStringHeight() can return a secret value during combat in 12.0.0+
        local height = self.Text:GetLineHeight()

        -- Only try GetStringHeight() if we're sure it's safe to access
        local stringHeight = self.Text:GetStringHeight()
        if canaccessvalue(stringHeight) and stringHeight >= 1 then
            height = stringHeight
        end
