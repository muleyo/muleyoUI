local Core = mUI:NewModule("mUI.Core")

-- Enable Module
Core:Enable()

function Core:OnEnable()
    -- Libraries
    local Config = LibStub("AceConfig-3.0")

    -- Get Modules
    local Gui = mUI:GetModule("mUI.Config.Gui")
    local General = mUI:GetModule("mUI.Config.Layouts.General")
    local Actionbars = mUI:GetModule("mUI.Config.Layouts.Actionbars")
    local Unitframes = mUI:GetModule("mUI.Config.Layouts.Unitframes")
    local Castbars = mUI:GetModule("mUI.Config.Layouts.Castbars")
    local Nameplates = mUI:GetModule("mUI.Config.Layouts.Nameplates")
    local Tooltips = mUI:GetModule("mUI.Config.Layouts.Tooltips")
    local MapMinimap = mUI:GetModule("mUI.Config.Layouts.MapMinimap")
    local Chat = mUI:GetModule("mUI.Config.Layouts.Chat")
    local Misc = mUI:GetModule("mUI.Config.Layouts.Misc")
    local Profiles = mUI:GetModule("mUI.Config.Layouts.Profiles")
    local About = mUI:GetModule("mUI.Config.Layouts.About")
    local Theme = mUI:GetModule("mUI.Modules.General.Theme")

    -- Enable Modules
    General:Enable()
    Actionbars:Enable()
    Unitframes:Enable()
    Castbars:Enable()
    Nameplates:Enable()
    Tooltips:Enable()
    MapMinimap:Enable()
    Chat:Enable()
    Misc:Enable()
    Profiles:Enable()
    About:Enable()
    Theme:Enable()

    -- Register Options
    Config:RegisterOptionsTable("mUIOptions_General_Tab", General:GetOptions())
    Config:RegisterOptionsTable("mUIOptions_Actionbars_Tab", Actionbars:GetOptions())
    Config:RegisterOptionsTable("mUIOptions_Unitframes_Tab", Unitframes:GetOptions())
    Config:RegisterOptionsTable("mUIOptions_Castbars_Tab", Castbars:GetOptions())
    Config:RegisterOptionsTable("mUIOptions_Nameplates_Tab", Nameplates:GetOptions())
    Config:RegisterOptionsTable("mUIOptions_Tooltips_Tab", Tooltips:GetOptions())
    Config:RegisterOptionsTable("mUIOptions_MapMinimap_Tab", MapMinimap:GetOptions())
    Config:RegisterOptionsTable("mUIOptions_Chat_Tab", Chat:GetOptions())
    Config:RegisterOptionsTable("mUIOptions_Misc_Tab", Misc:GetOptions())
    Config:RegisterOptionsTable("mUIOptions_Profiles_Tab", Profiles:GetOptions())
    Config:RegisterOptionsTable("mUIOptions_About_Tab", About:GetOptions())
end

function Core:OnInitialize()
    -- Get Player Class, Colors and Theme
    local _, class = UnitClass("player")
    local classColor = RAID_CLASS_COLORS[class]
    local customColor = mUI.db.profile.general.color
    local themes = {
        Disabled = { 1, 1, 1 },
        Dark = { 0.3, 0.3, 0.3 },
        Class = { classColor.r, classColor.g, classColor.b },
        Custom = { customColor[1], customColor[2], customColor[3], customColor[4] },
    }
    local theme = themes[mUI.db.profile.general.theme]

    -- Debug Print Function
    function mUI:Debug(msg)
        print("|cff009cffmuleyo|rUI:", msg)
    end

    -- Version check
    local currentVersion = C_AddOns.GetAddOnMetadata("mUI", "version")

    local function GetDefaultCommChannel()
        if IsInRaid() then
            return IsInRaid(LE_PARTY_CATEGORY_INSTANCE) and "INSTANCE_CHAT" or "RAID"
        elseif IsInGroup() then
            return IsInGroup(LE_PARTY_CATEGORY_INSTANCE) and "INSTANCE_CHAT" or "PARTY"
        elseif IsInGuild() then
            return "GUILD"
        else
            return "YELL"
        end
    end

    function mUI:ReceiveVersion(_, version, _, sender)
        if not mUI.db.profile.new_version then
            if (version > currentVersion) then
                mUI:Debug(
                    "A newer version is available. If you experience any errors or bugs, updating is highly recommended.")
                mUI.db.profile.new_version = version
            end
        elseif (mUI.db.profile.new_version == currentVersion) or (mUI.db.profile.new_version <= currentVersion) then
            mUI.db.profile.new_version = false
        end
    end

    function mUI:SendVersion(channel)
        self:SendCommMessage("mUIVersion", currentVersion, channel or GetDefaultCommChannel())
    end

    mUI:RegisterComm("mUIVersion", "ReceiveVersion")

    -- Link Popup
    function mUI:Link(url)
        if not url then return end
        StaticPopupDialogs["mUIPopup"] = nil
        StaticPopupDialogs["mUIPopup"] = {

            text = "|cff009cffmuleyo|rUI\n\n|cffffcc00Copy the link below ( CTRL + C )|r",
            button1 = CLOSE,
            whileDead = true,
            hideOnEscape = true,
            hasEditBox = true,
            editBoxWidth = 200,
            EditBoxOnEscapePressed = function(self)
                self:GetParent():Hide()
            end,
            OnShow = function(self, data)
                self.editBox:SetText(data.url)
                self.editBox:HighlightText()
                self.editBox:SetScript("OnKeyDown", function(_, key)
                    if key == "C" and IsControlKeyDown() then
                        C_Timer.After(0.3, function()
                            self.editBox:GetParent():Hide()
                            UIErrorsFrame:AddMessage("Link copied to clipboard")
                        end)
                    end
                end)
            end,
        }
        StaticPopup_Show("mUIPopup", "", "", { url = url })
    end

    -- Theme Functions
    function mUI:Color(sub, alpha)
        -- Update Custom Color & Theme
        customColor = mUI.db.profile.general.color
        themes["Custom"] = { customColor[1], customColor[2], customColor[3], customColor[4] }
        theme = themes[mUI.db.profile.general.theme]

        if theme then
            if theme[4] then alpha = theme[4] end
            if not alpha then alpha = 1 end
            local color = { 0, 0, 0, alpha }
            if sub then
                color[1] = theme[1] - sub
                color[2] = theme[2] - sub
                color[3] = theme[3] - sub
            end
            return color
        end
    end

    function mUI:Skin(frame, isTable, isGui)
        local forbiddenFrames = mUI:GetModule("mUI.Modules.General.Theme").forbiddenFrames
        -- Update Custom Color & Theme
        customColor = mUI.db.profile.general.color
        themes["Custom"] = { customColor[1], customColor[2], customColor[3], customColor[4] }
        theme = themes[mUI.db.profile.general.theme]

        if frame then
            if isGui then
                for _, v in pairs({ frame:GetRegions() }) do
                    if v:GetObjectType() == "Texture" then
                        v:SetDesaturated(true)
                        v:SetVertexColor(0.15, 0.15, 0.15, 1)
                    end
                end
            elseif not isTable then
                for _, v in pairs({ frame:GetRegions() }) do
                    if (not forbiddenFrames[v:GetName()]) and (not forbiddenFrames[v]) then
                        if v:GetObjectType() == "Texture" then
                            if themes["Disabled"] == theme then
                                v:SetDesaturated(false)
                            else
                                v:SetDesaturated(true)
                            end
                            v:SetVertexColor(unpack(mUI:Color(0.15)))
                        end
                    end
                end
            else
                for _, v in pairs(frame) do
                    if v then
                        if themes["Disabled"] == theme then
                            v:SetDesaturated(false)
                        else
                            v:SetDesaturated(true)
                        end
                        v:SetVertexColor(unpack(mUI:Color(0.15)))
                    end
                end
            end
        end
    end
end
